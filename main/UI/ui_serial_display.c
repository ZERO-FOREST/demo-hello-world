...
static esp_err_t init_psram_buffer(void)
{
    ...
    ESP_LOGI(TAG, "PSRAM buffer initialized: %d lines, %d bytes", 
             MAX_LINES, MAX_LINES * sizeof(display_line_t));
    return ESP_OK;
}
...
{
    if (!g_text_area || !g_buffer_initialized || g_lines == NULL) return;
...
{
    if (!g_buffer_initialized || g_lines == NULL) return;
...
void ui_serial_display_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(g_back_btn, 100, 40);
    ...
    lv_obj_set_style_shadow_width(g_back_btn, 3, 0);
