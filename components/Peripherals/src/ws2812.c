...
static size_t ws2812_encode(rmt_encoder_t *encoder, rmt_channel_handle_t channel,
    ...
    case 0: // 发送RGB数据
        encoded_symbols += ws2812_encoder->bytes_encoder->encode(ws2812_encoder->bytes_encoder, channel, primary_data, data_size, &session_state);
    ...
    case 1: // 发送复位信号
        encoded_symbols += ws2812_encoder->copy_encoder->encode(ws2812_encoder->copy_encoder, channel, &s_ws2812_reset, sizeof(s_ws2812_reset), &session_state);
        if (session_state & RMT_ENCODING_COMPLETE) {
...
ws2812_color_t ws2812_scale_color(ws2812_color_t color, uint8_t scale) {
    ws2812_color_t scaled = {
        .red = (color.red * scale) / 255,
        .green = (color.green * scale) / 255,
        .blue = (color.blue * scale) / 255
    };
    return scaled;
}
