...
static esp_err_t lsm6ds3_i2c_init(void)
{
    ...
    i2c_master_bus_config_t i2c_mst_config = {
        ...
        .i2c_port = LSM6DS3_I2C_PORT,
        .scl_io_num = LSM6DS3_I2C_SCL_PIN,
        .sda_io_num = LSM6DS3_I2C_SDA_PIN,
        .glitch_ignore_cnt = 7,
        .flags.enable_internal_pullup = true,
    };
    ret = i2c_new_master_bus(&i2c_mst_config, &g_lsm6ds3_handle.i2c_bus_handle);
    if (ret != ESP_OK) {
    ...
    i2c_device_config_t dev_cfg = {
        ...
        .scl_speed_hz = LSM6DS3_I2C_FREQ_HZ,
    };
    ret = i2c_master_bus_add_device(g_lsm6ds3_handle.i2c_bus_handle, &dev_cfg, &g_lsm6ds3_handle.i2c_dev_handle);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C master bus add device failed: %s", esp_err_to_name(ret));
        i2c_del_master_bus(g_lsm6ds3_handle.i2c_bus_handle);
        return ret;
    }

    g_lsm6ds3_handle.i2c_port = LSM6DS3_I2C_PORT;
...
static esp_err_t lsm6ds3_read_reg_i2c(uint8_t reg, uint8_t *data, size_t len)
{
    esp_err_t ret = i2c_master_transmit_receive(g_lsm6ds3_handle.i2c_dev_handle, &reg, 1, data, len, pdMS_TO_TICKS(100));
    if (ret != ESP_OK) {
...
esp_err_t lsm6ds3_deinit(void)
{
    ...
    if (g_lsm6ds3_handle.i2c_bus_handle) {
        i2c_del_master_bus(g_lsm6ds3_handle.i2c_bus_handle);
    }
#else
...
{
    return g_lsm6ds3_handle.is_initialized;
}
