...
static esp_err_t lsm6ds3_i2c_init(void)
{
    ...
    i2c_config_t conf = {
        ...
        .master.clk_speed = LSM6DS3_I2C_FREQ_HZ,
    };
    
    // 初始化I2C
    ret = i2c_param_config(LSM6DS3_I2C_PORT, &conf);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C param config failed: %s", esp_err_to_name(ret));
        return ret;
    }
    
    ret = i2c_driver_install(LSM6DS3_I2C_PORT, conf.mode, 0, 0, 0);
    if (ret != ESP_OK) {
...
static esp_err_t lsm6ds3_read_reg_i2c(uint8_t reg, uint8_t *data, size_t len)
{
    ...
    i2c_master_write_byte(cmd, (LSM6DS3_I2C_ADDR << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, reg, true);
    
    // 读取数据
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (LSM6DS3_I2C_ADDR << 1) | I2C_MASTER_READ, true);
    if (len > 1) {
    ...
    ret = i2c_master_cmd_begin(LSM6DS3_I2C_PORT, cmd, pdMS_TO_TICKS(100));
...
#if LSM6DS3_USE_I2C
    i2c_driver_delete(LSM6DS3_I2C_PORT);
#else
