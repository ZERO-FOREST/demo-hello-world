...
#include "esp_event.h"
#include "esp_heap_caps.h" // Added for MALLOC_CAP_SPIRAM
#include "esp_log.h"
#include "esp_netif.h"
#include "esp_wifi.h"
#include "freertos/FreeRTOS.h"
#include "freertos/event_groups.h"
#include "heap_caps.h" // Added for heap_caps_malloc
#include "lwip/dns.h"
...
static uint8_t* s_data_buffer = NULL;
...
bool tcp_client_init(void) {
    ...
    recv_buffer = heap_caps_malloc(RECV_BUFFER_SIZE, MALLOC_CAP_SPIRAM);
    frame_buffer = heap_caps_malloc(FRAME_BUFFER_SIZE, MALLOC_CAP_SPIRAM);
    s_data_buffer = heap_caps_malloc(RECV_BUFFER_SIZE, MALLOC_CAP_SPIRAM);

    if (!recv_buffer || !frame_buffer || !s_data_buffer) {
        ESP_LOGE(TAG, "Failed to allocate buffers from PSRAM");
        free(recv_buffer);
        free(frame_buffer);
        free(s_data_buffer);
        return false;
...
client_state_t tcp_client_get_state(void) { return client_state; }
...
bool tcp_client_process_received_data(void) {
    ...
    if (!s_data_buffer || !recv_buffer) {
        ESP_LOGE(TAG, "Receive buffers not initialized");
        client_state = CLIENT_STATE_ERROR;
        return false;
    }
    ...
    // 将新数据添加到缓冲区
    if (s_buffer_pos + received < RECV_BUFFER_SIZE) {
        memcpy(&s_data_buffer[s_buffer_pos], recv_buffer, received);
        s_buffer_pos += received;
    } else {
        // 缓冲区溢出，重置
        ESP_LOGW(TAG, "接收缓冲区溢出，重置缓冲区");
        memcpy(s_data_buffer, recv_buffer, received);
        s_buffer_pos = received;
    }

    // 处理缓冲区中的数据帧
    while (s_buffer_pos >= MIN_FRAME_SIZE) {
        // 寻找帧头
        int header_pos = find_frame_header(s_data_buffer, s_buffer_pos);
        if (header_pos == -1) {
            // 没找到帧头，清空缓冲区
            s_buffer_pos = 0;
            break;
        ...
        if (header_pos > 0) {
            // 丢弃帧头前的无效数据
            memmove(s_data_buffer, &s_data_buffer[header_pos], s_buffer_pos - header_pos);
            s_buffer_pos -= header_pos;
        }
        ...
        // 获取帧长度
        uint8_t frame_length_field = s_data_buffer[2];
        uint16_t total_frame_size = 2 + 1 + frame_length_field + 2; // 帧头+长度+内容+CRC

        if (s_buffer_pos < total_frame_size) {
            // 数据不完整，等待更多数据
            ESP_LOGD(TAG, "数据不完整, 需要%d字节, 现有%d字节", total_frame_size, s_buffer_pos);
            break;
        }

        // 解析数据帧
        protocol_frame_t frame;
        parse_result_t result = parse_protocol_frame(s_data_buffer, total_frame_size, &frame);
        ...
        // 移除已处理的帧
        memmove(s_data_buffer, &s_data_buffer[total_frame_size], s_buffer_pos - total_frame_size);
        s_buffer_pos -= total_frame_size;
    }
...
void print_received_frame(const protocol_frame_t* frame) {
    ...
    case FRAME_TYPE_REMOTE_CONTROL: {
        ...
        for (uint8_t i = 0; i < rc->channel_count && i < MAX_CHANNELS; i++) {
            if (i == 0)
                ESP_LOGI(TAG, "  油门=%d", rc->channels[i]);
            else if (i == 1)
                ESP_LOGI(TAG, "  方向=%d", rc->channels[i]);
            else
                ESP_LOGI(TAG, "  CH%d=%d", i + 1, rc->channels[i]);
        }
        break;
    }
    ...
    while (1) {
        ...
        case CLIENT_STATE_CONNECTED: {
            ...
            if (current_time - last_telemetry_send >= pdMS_TO_TICKS(1000)) {
                ...
                telemetry_data_payload_t telemetry = {.voltage_mv = sim_telemetry.voltage_mv,
